...
The parameters of the RNADE can be learnt by performing gradient ascent on the log-likelihood of the training set. 
...
\begin{figure}
	\centering
    \includegraphics[scale=0.65]{RNN_RNADE.png}
    \caption{Awesome RNN}
    \label{fig:rnn}
\end{figure}
...
For training the RNN-RNADE, we minimise the negative log-likelihood cost of observing a sequence $\mathbf{x}$. The cost function is given by:

\begin{align*}
L(\theta) &= - \log P(\mathbf{x})\\
&= -\sum_{t=1}^{T} \log P(\mathbf{x}^t;\theta^t)\\
\end{align*}
...
Suppose we obtain $ \frac{\partial L}{\partial \mathbf{b}^{\alpha}_{t}}, \frac{\partial L}{\partial \mathbf{b}^{\mu}_{t}}, \frac{\partial L}{\partial \mathbf{b}^{\sigma}_{t}}$, then the gradients with respect to the other model parameters can be calculated as follows:
