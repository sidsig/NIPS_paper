...
\begin{abstract}
We propose a tractable probabilistic model for generating complex high dimensional sequences. This model combines a powerful real-valued distribution estimator with a Recurrent Neural Network (RNN) for capturing temporal dependencies in complex, high-dimensional data. Unlike other models in this family, computing probabilities for sequences and sampling from these models is tractable and simple. Therefore the model can be used in combination with other sequential models and not just for generating sequences. We demonstrate the advantages of the model by comparing it on two standard tasks. The 
\end{abstract}
